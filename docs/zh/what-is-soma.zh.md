...
这些都在 roadmap 里。`soma_actor` 的 agent/entity 语义已经建好，v0.5 的 LLM worker、proposal、policy、budget 和 actor-to-actor message 也已经落地；v0.6 让事件流可读、可选持久化；v0.7.1-v0.7.5 已经实现了持久化 run journal、reconstruct、resume plan、手动 resume executor、interrupted-run discovery 和 boot auto-resume。后续 runtime 工作才考虑 DAG / parallel execution。
...
- `agent-shell.zh.md` 是 agent shell 设计（已实现）：外部 agent 只知道 `soma` 一个工具，Soma 内部完成工具探索、help、搜索、probe、Soma Lisp 编写、编译、运行和监控。探索层（AS.1–AS.4）、版本化运行时服务（RS.1，`../service-contract.md`）与 `soma.delegate` 委托层（AS.5a/AS.5）均已合入主干。
- `../design.md` 解释设计原则和 OTP 进程模型。
- `../usage.md` 是用户手册，记录 `soma` 命令、任务文件、任务管理、trace、模型配置和排错。
...
- `../roadmap.md` 记录包括 v0.7.5 在内的当前已完成层、后续未来层，以及 node B / CLI / Lisp 轨道状态。
