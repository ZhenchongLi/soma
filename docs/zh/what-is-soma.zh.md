...
isolation）先建好并验证；`soma_actor` 这一层也已经建在它之上（v0.4，最小切片：
固定规则决策，还没接真正的 LLM）。它要证明的观点是：
...
在 Soma 里，步骤列表只描述“要运行什么”。真正的运行语义，包括超时、取消、监控、崩溃隔离、进程生命周期和事件记录，都交给 Erlang/OTP 的运行时机制处理。`soma_actor` 已经把这条执行路径纳入自己的消息处理流程：接收消息（`send`/`ask`）、建任务、按固定规则启动它自己拥有的 `soma_run`、观察终态消息、记录结果或在失败/超时/取消时作为数据存活。v0.4 还没接真正的 LLM——调用 LLM 做规划/响应、再经过 policy gate 校验，是 v0.5。
...
这些都在 roadmap 里。`soma_actor` 的 agent/entity 语义已经建好（v0.4，固定规则、无真实 LLM）；下一步是在它上面加 LLM planner 和 policy gate（v0.5），再往后才是 DAG、actor-driven planning 和持久化 resume。
...
- `../contracts/` 下的 v0.2 / v0.3 / v0.4 测试契约把每个行为保证映射到具体测试（`v0.4-test-contract.md` 是 `soma_actor` 的 12 条进程行为证明）。
- `../roadmap.md` 记录 v0.4 之后才应该进入的未来层。
