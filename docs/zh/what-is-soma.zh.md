...
这些都在 roadmap 里。`soma_actor` 的 agent/entity 语义已经建好，v0.5 的 LLM worker、proposal、policy、budget 和 actor-to-actor message 也已经落地；v0.6 让事件流可读、可选持久化。下一条主线是持久化 resume，之后才考虑 DAG / parallel execution。
...
- `../contracts/` 下的 v0.2-v0.6、L.1-L.5 和 CLI 契约把每个行为保证映射到具体测试。
- `../roadmap.md` 记录 v0.7 及之后的未来层，以及 node B / CLI / Lisp 轨道状态。
