...
| If the registry loses certainty while reading/persisting edge acceptance, the real socket response is `admission_in_doubt` with the minted task/run/correlation ids and never `accepted`; releasing the store gate drives the pending retry to exactly one acceptance, run-side commit, and tool execution under the same queryable task id. | `soma_cli_resume_SUITE` | `test_acceptance_unknown_socket_returns_ids_and_retries_once` |
| If the run-owned commit blocks until the admission lease expires, the real socket response is `admission_in_doubt` with all three ids and never `accepted`; releasing the commit may durably record it, but the expired paused run cancels before every step, tool, and state effect, and the same task id projects cancelled. | `soma_cli_resume_SUITE` | `test_activation_unknown_socket_returns_ids_without_effect` |
...
| Live-run adoption preserves the exact admission identity and committed state; if an older generation's matching rejection lands afterward and that run is interrupted, recovery re-reads the absorbing tombstone and reaches cancellation without a resumed attempt, step/tool boundary, or state effect while the replacement daemon stays alive. | `soma_cli_resume_SUITE` | `test_late_rejection_after_live_adoption_cancels_without_replay` |
