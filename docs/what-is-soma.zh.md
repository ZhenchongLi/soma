...
- `docs/erlang-otp-primer.zh.md` 面向非 Erlang 读者解释 BEAM、process、mailbox、OTP、`gen_server`、`gen_statem`、supervisor、application、release 等概念。
- `docs/soma-actor-final-design.zh.md` 是 `soma_actor` 的最终设计说明：actor 语义、message loop、decision pipeline、policy gate、预算、结果、事件和最小切片。
- `docs/soma-actor.zh.md` 解释最核心的 `soma_actor` 工作模型：如何通过 message 触发、如何调用 LLM/run/tool、如何通过 reply/events/correlation_id 获取结果。
- `docs/design.md` 解释设计原则和 OTP 进程模型。
- `docs/usage.md` 记录实际 API、步骤格式、事件读取和取消方式。
- `docs/tool-manifest.md` 记录工具 manifest 和 CLI adapter 协议。
- `docs/v0.2-test-contract.md` 把每个行为保证映射到具体测试。
- `docs/roadmap.md` 记录 v0.3 之后才应该进入的未来层。
