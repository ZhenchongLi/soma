...
| Identifier semantics | Step ids and `from_step` references are caller-defined identifiers: the symbol spelling `(id my-step)` and the string spelling `(id "my-step")` are equivalent, arrive as binary identifiers in results, events, and traces, and never depend on VM atom-table state — a fresh spelling is accepted identically on a fresh or warm server and interns no atom. Tool names resolve against the registered tool vocabulary. Argument names resolve against the resolved tool's declared params; an argument name outside that vocabulary rejects the invocation typed (`reason_class` `invalid_args`) before any run starts. |
| Binary values | A binary that is valid UTF-8 uses the Lisp string form. Any other binary uses the lossless `(bytes (hex "<uppercase hexadecimal>"))` form; clients decode its even-length uppercase hexadecimal payload to recover the exact bytes. This applies to inline results, an artifact descriptor's `truncated_inline` value, and watch event fields. A real text value beginning with `base64:` remains ordinary text and is never a byte sentinel. |
