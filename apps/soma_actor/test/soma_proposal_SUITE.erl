...
reply_proposal_stored_as_task_result(_Config) ->
    ...
    %% v0.5.4: an approved toolless `reply' proposal has nothing to run, so it
    %% reaches `completed' (not resting at `approved') with the normalized
...
reply_proposal_emits_proposal_created_with_correlation_id(_Config) ->
    ...
    Envelope = #{type => <<"chat">>,
                 ...
                 llm => Llm},
    {ok, TaskId} = soma_actor:send(ActorPid, Envelope),
    %% v0.5.4: an approved toolless `reply' proposal reaches `completed'.
    ok = wait_for_status(ActorPid, TaskId, completed, 100),
