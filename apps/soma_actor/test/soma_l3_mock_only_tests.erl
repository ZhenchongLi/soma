-module(soma_l3_mock_only_tests).

-include_lib("eunit/include/eunit.hrl").

-define(SUITE_PATH, "apps/soma_actor/test/soma_actor_lisp_proposal_SUITE.erl").

%% Issue #109 criterion 9: the L.3 test run opens no real LLM call and no
%% external network socket -- mock LLM only. This is a by-construction invariant
%% of `soma_actor_lisp_proposal_SUITE': every LLM directive it drives the actor
%% with is the `proposal' mock directive (the same mock the v0.5, L.1 and L.2
%% suites use, which returns a pre-built proposal without `perform_call/1'
%% reaching `soma_llm_openai'), and no L.3 test opt carries a real-provider
%% config. This guard pins that invariant against the actual suite source so a
%% later edit that slips a real provider (or a non-`proposal' directive) into the
%% L.3 run is caught by the gate. Modeled on `soma_l2_mock_only_tests'.
